...
1. **选择远程分支**：从可用的远程分支中选择（或使用 `-b` 选项）。
2. **选择标签位置**（如适用）：如果所选分支已被合并到 main/master/develop，系统会提示您选择在哪里创建标签：
   - 在源分支上（您选择的分支）
   - 在已合并的分支上（例如 main）- 使用合并提交
3. **选择后缀**：选择一个后缀（例如：`test` 或 `main`）。
4. **输入消息**：添加可选的标签注释/消息。
5. **确认推送**：确认是否要将标签推送到远程仓库 `origin`。
6. **最终审查**：创建前会显示标签名称预览，供您进行最终确认。
